...
export function BrandKitEditor({
  ...
  async function save() {
    ...
    try {
      ...
      if (!res.ok) {
        setError(json.error ?? "Could not save Brand Kit changes.");
        return;
      }
  ...
  async function download() {
    ...
    try {
      ...
      if (!res.ok) {
        const json = await res.json().catch(() => ({}));
        setError(json.error ?? "Export failed. Assets may be missing.");
        return;
      }
      ...
      setMessage("Export pack downloaded.");
    } catch {
      setError("Could not download assets. Check your connection and try again.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function deliver() {
    ...
    try {
      ...
      if (typeof json.returnUrl === "string" && json.returnUrl.startsWith("http")) {
        window.location.href = json.returnUrl;
      }
    } catch {
      setError("Could not deliver to Aleya. Check your connection and try again.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
      <section className="panel animate-rise rounded-3xl p-6">
        <h1 className="text-3xl break-words">{kit.business_name}</h1>
        ...
        <div className="mt-6 grid gap-4">
          ...
          <p className="text-sm text-black/60 break-words">Prompt: {kit.logo_prompt}</p>
        </div>
        <div className="mt-6 flex flex-wrap gap-2">
          ...
          <button type="button" className="btn btn-secondary" disabled={busy} onClick={deliver}>
            Send to Aleya
          </button>
        </div>
        {message ? (
          <p className="mt-4 text-sm text-[var(--forest)]" role="status">
            {message}
          </p>
        ) : null}
        {error ? (
